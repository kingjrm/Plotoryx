...
$query = "SELECT * FROM entries WHERE user_id = ? AND type = 'manhwa'";
$params = [$userId];
...
if ($filter == 'ongoing') {
    ...
    // Assuming we add a favorites field later, for now just show all
}
...
<div class="flex justify-between items-center mb-6">
    ...
    <div class="flex space-x-2">
        <a href="?page=manhwa&filter=all" class="px-3 py-1 text-sm bg-gray-200 rounded <?php echo $filter == 'all' ? 'bg-indigo-600 text-white' : ''; ?>">All</a>
        <a href="?page=manhwa&filter=ongoing" class="px-3 py-1 text-sm bg-gray-200 rounded <?php echo $filter == 'ongoing' ? 'bg-indigo-600 text-white' : ''; ?>">Ongoing</a>
